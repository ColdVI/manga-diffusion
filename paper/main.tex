...
Our results show that \emph{[findings to be inserted after evaluation]}, with implications for how practitioners should approach annotation when fine-tuning on niche visual styles.
\end{abstract}
...
\end{tabular}
\vspace{0.5em}\\
{\small \textit{To be filled after evaluation runs complete.}}
\end{table}
...
\paragraph{Findings.}
\textit{[To be written after evaluation. Anticipated structure: state which condition achieves lower FID and higher DINOv2 similarity, discuss why structured tags or free-form captions better capture the stylistic vocabulary of noir manga, note any CLIP score trade-off.]}

\paragraph{Why captioning matters.}
The cross-attention mechanism in a latent diffusion model learns to associate image regions with text tokens; the granularity and domain-specificity of those tokens directly determines what the LoRA can adapt.
BLIP-2 captions, trained on photorealistic web data, may produce tokens that the base SDXL model already handles well, offering little signal for style-specific adaptation.
WD14 tags, derived from the Danbooru illustration community, supply the stylistic vocabulary in a form the model can bind directly to the trigger token.
We expect this difference to manifest most clearly in DINOv2 similarity, which is sensitive to texture and line quality rather than semantic content.

\paragraph{Limitations.}
Our study is constrained by the size of Manga109-s and our selection of noir-style works from it; the 200-panel training set is small relative to typical LoRA training corpora.
We study a single sub-genre; whether the findings generalize to shonen action, shojo romance, or other manga aesthetics remains open.
Compute constraints limit us to rank-32 experiments for the primary comparison; the rank ablation (16, 64) is deferred to supplementary material.

\paragraph{Conclusion.}
We have presented a controlled study of captioning strategy as a determinant of LoRA style fidelity, using noir manga as a test domain.
By holding all hyperparameters constant and varying only the caption source, we isolate the annotation step as an independent variable.
We release our curation code, caption files, and evaluation scripts to facilitate reproducibility and future comparisons against additional captioning strategies, including structured prompting via large language models.
