% CVPR Workshop 4-page format (self-contained, no cvpr.sty needed)
% Target: Workshop on Generative Models for Creative Content, CVPR 2026
\pdfoutput=1  % force pdflatex so PNG/JPG graphics work
\documentclass[10pt,twocolumn,letterpaper]{article}

\usepackage[top=2.5cm,bottom=2.5cm,left=1.9cm,right=1.9cm,columnsep=0.8cm]{geometry}
\usepackage{times}
...
% Title formatting — tight spacing for two-column workshop format
\usepackage{titlesec}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth,height=0.40\textheight,keepaspectratio]{figures/qual_grid.png}
  \caption{Generated images for three prompts (rows) across all conditions (columns), seed 0.
  \textbf{Left}: baseline SDXL produces photorealistic color images with no manga character.
  \textbf{Center}: BLIP-2 LoRA produces ink linework, screentone, and panel-appropriate compositions.
  \textbf{Right}: WD14 LoRA achieves monochrome manga style with looser character rendering.}
  \label{fig:qual}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/metrics_bar.pdf}
  \caption{Metric comparison across all three conditions. Bold outlines mark the winner per metric. BLIP-2 wins on style fidelity (FID, DINOv2); WD14 wins on text-prompt alignment (CLIP).}
  \label{fig:metrics}
\end{figure}

\subsection{Caption Comparison}

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/caption_compare.pdf}
  \caption{Three training panels with their auto-generated captions under each condition. BLIP-2 produces compositionally rich sentences; WD14 outputs flat tag lists. The same trigger token \texttt{gknoir style,} is prepended to both before training.}
  \label{fig:captions}
\end{figure}
...
\noindent\textbf{Why BLIP-2 wins on style.}
