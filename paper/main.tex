...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth,height=0.42\textheight,keepaspectratio]{figures/qual_grid.png}
  \caption{Qualitative comparison for three prompts (rows) across all conditions (columns), fixed seed 0.
  \textbf{Left}: baseline SDXL generates photorealistic color images with no manga character.
  \textbf{Center}: BLIP-2 LoRA produces consistent ink linework, screentone texture, and panel-appropriate compositions.
  \textbf{Right}: WD14 LoRA achieves monochrome manga style but with looser character rendering — note the distant silhouette (bottom row) vs.\ BLIP-2's foregrounded figure.}
  \label{fig:qual}
\end{figure}
